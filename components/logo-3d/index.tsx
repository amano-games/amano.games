...
type LogoGLTF = {
  nodes: {
    Hand_Bone: THREE.Object3D;
    eye_control: THREE.Object3D;
    Pupil_Control: THREE.Object3D;
    a1: THREE.Object3D;
    m: THREE.Object3D;
    a2: THREE.Object3D;
    n: THREE.Object3D;
    o: THREE.Object3D;
    single: THREE.Object3D;
    text_deform_control: THREE.Object3D;
    eye: THREE.SkinnedMesh;
    Hand: THREE.SkinnedMesh;
    Pupil: THREE.SkinnedMesh;
    Text: THREE.SkinnedMesh;
  };
  materials: {
...
export default function Logo3D(props: ThreeElements['group']) {
  const group = useRef<THREE.Group>(null);
  const pupil = useRef<THREE.Object3D>(null);
  const pupilEnabled = useRef(false);
  const scale: [number, number, number] = [1.5, 1.5, 1.5];

  const { nodes, animations, materials } = useGLTF(
    '/logo.gltf'
  ) as unknown as LogoGLTF & { animations: THREE.AnimationClip[] };
    ...
    (e: THREE.Event & { action: THREE.AnimationAction }) => {
      ...
      switch (name) {
        case 'Opening':
          ...
          nextAction = actions.Text ?? null;
          break;
        case 'Text':
          nextAction = actions.idle ?? null;
          break;
        case 'idle':
          break;
        default:
  ...
  useEffect(() => {
    actions.Opening?.setLoop(THREE.LoopOnce, 1);
    if (actions.Opening) {
      actions.Opening.clampWhenFinished = true;
    }
    if (actions.Text) {
      actions.Text.clampWhenFinished = true;
      actions.Text.setLoop(THREE.LoopOnce, 1);
    }
  ...
  useFrame((state) => {
    if (!pupilEnabled.current || !pupil.current) return;
